...
class Login {
	...
	main = async (page) => {
		...
		if (!is_logged_in) {
			...
			if (auth2FA) await this.process2FA(page);
		}

		this.logger.info('Login.main():: {LOGGED IN}');
	};
	...
	async isLoggedIn(page) {
		...
		return await page.evaluate(async () => {
			return await new Promise((resolve) => {
				...
				if (headerNavigation.length > 0) return resolve(true);
	...
	async process2FA(page) {
		...
		await page.waitForNavigation({ timeout: this.browserTimeout * 4 });
		await page.screenshot({ path: `${this.dir}process2FA.png` });
	}
}
