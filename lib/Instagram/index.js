...
class Instagram {
    constructor(dir) {
        ...
        this.logger = createLogger(this.dir);
    ...
    async getIncognitoPageInstance() {
        const browser = await puppeteer.launch({
            ...
            headless: true,
            userDataDir: `${this.dir.replace(/\/+$/, '')}/puppeteerChromeSessionInstagram`
        });
...
const createLogger = (_dir) => {

    const dir = _dir.replace(/\/+$/, '') + "/logs"
    ...
    const logger = winston.createLogger({
        ...
        transports: [
            new winston.transports.File({ filename: `${dir.replace(/\/+$/, '')}/stderr.log`, level: 'error' }),
            new winston.transports.File({ filename: `${dir.replace(/\/+$/, '')}/stdout.log`, level: 'info' }),
            new winston.transports.File({ filename: `${dir.replace(/\/+$/, '')}/std.log` }),
        ],
    });
