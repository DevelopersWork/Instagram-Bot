...
class Post {
	...
	main = async (page, postDetails) => {
		this.logger.debug('Post.main():: {STARTED}');
			...
			try {
				if(this.limited) throw Error('LIMITED FOR THE DAY');
				...
				await page.screenshot({ path: `${this.dir}error.png` });
				return Promise.reject(error);
			}
		await page.screenshot({ path: `${this.dir}main.png` });
	...
	async publishPost(page, username, postDetails, type = 'video') {
		...
		if (await page.$('[class="_acan _acap _acas"]') !== null) {
    		let message = await page.$('[class="_aacl _aacr _aact _aacx _aad6 _aadb"]')
			message =  await message.getProperty('textContent')
			message = await message.jsonValue()
			if(message === "Your post could not be shared. Please try again."){
				this.limited = true;
				throw Error(message)
			}
  		}
	...
	async uploadVideo(page, username, postDetails) {
		...
		await page.waitForSelector('button[class="_acan _acao _acas _aj1-"]', {
